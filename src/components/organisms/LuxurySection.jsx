import SectionText from "../molecules/SectionText"
const LuxurySection = () => {
  return (
    <div className="luxury-section">
      <div className="luxury-section-content">
        <SectionText
          tag1="h3"
          tag2="h2"
          tag3="p"
          text1="luxury section"
          text2="Perfect circles andRectangles"
          text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy  text ever since the 1500s, when an unkno"
        />
        <div className="services-container">
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          <SectionText
            tag1="h4"
            tag2="h7"
            tag3="p"
            text1="Easy Interface"
            text2="e"
            text3="Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever sinc"
          />
          
        </div>
      </div>
    </div>
  );
}
export default LuxurySection